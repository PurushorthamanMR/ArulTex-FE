...
function toBackend(body, isUpdate = false) {
  ...
  // Barcode is create-once only (generated by backend). Never send barCode on update.
  if (!isUpdate) {
    const barcodeVal = body.barcode ?? body.barCode
    if (barcodeVal != null && String(barcodeVal).trim() !== '') payload.barCode = String(barcodeVal).trim()
  }
  return payload
}
...
/** List products for barcode label page (id, productName, barCode). */
export async function listForBarcode() {
  const res = await fetch(`${BASE_URL}/product/listForBarcode`, {
    method: 'GET',
    headers: getAuthHeaders()
  })
...
/** Update only barcode (from Barcode Labels page). Other product fields stay unchanged. */
export async function updateBarcodeOnly(id, barCode) {
  const payload = { id: Number(id), barCode: barCode != null && String(barCode).trim() !== '' ? String(barCode).trim() : null }
  const res = await fetch(`${BASE_URL}/product/updateBarcode`, {
    method: 'POST',
    headers: getAuthHeaders(),
    body: JSON.stringify(payload)
  })
